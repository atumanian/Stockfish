...
TTEntry* TranspositionTable::probe(const Key k, TTEntry::Data& ttData, bool& found) const {
  ...
  int entryValue = 16 * tte[0].data.is_zero_pos() + tte[0].data.depth() - (((0x103FF + generation8 - int(tte[0].data)) & 0xFC00) >> 7);
  for (int i = 1; i < ClusterSize; ++i) {
      int newValue = 16 * tte[i].data.is_zero_pos() + tte[i].data.depth() - (((0x103FF + generation8 - int(tte[i].data)) & 0xFC00) >> 7);
